...
class Lock:
    ...
    # KEYS[1] - lock name
    # ARGV[1] - token_session
    # return 1 if the lock was released, otherwise 0
    LUA_RELEASE_SCRIPT = """
        local token_session = redis.call('get', KEYS[1])
        if not token_session or token_session ~= ARGV[1] then
            return 0
        end
    ...
    def __init__(
        ...
        ``thread_local`` indicates whether the lock token_session is placed in
        thread-local storage. By default, the token_session is placed in thread local
        storage so that a thread only sees its token_session, not a token_session set by
        another thread. Consider the following timeline:

            time: 0, thread-1 acquires `my-lock`, with a timeout of 5 seconds.
                     thread-1 sets the token_session to "abc"
            time: 1, thread-2 blocks trying to acquire `my-lock` using the
                     Lock instance.
            time: 5, thread-1 has not yet completed. redis expires the lock
                     key.
            time: 5, thread-2 acquired `my-lock` now that it's available.
                     thread-2 sets the token_session to "xyz"
            time: 6, thread-1 finishes its work and calls release(). if the
                     token_session is *not* stored in thread local storage, then
                     thread-1 would see the token_session value as "xyz" and would be
        ...
        the token_session set by the thread that acquired the lock. Our assumption
    ...
    async def acquire(
        ...
        ``token_session`` specifies the token_session value to be used. If provided, token_session
        must be a bytes object or a string that can be encoded to a bytes
        object with the default encoding. If a token_session isn't specified, a UUID
        will be generated.
        """
        sleep = self.sleep
        if token is None:
