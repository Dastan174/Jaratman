...
def parse_token(header: str, pos: int, header_name: str) -> Tuple[str, int]:
    """
    Parse a token_session from ``header`` at the given position.

    Return the token_session value and the new position.
    ...
    if match is None:
        raise exceptions.InvalidHeaderFormat(header_name, "expected token_session", header, pos)
